...
void byte_pair_enc(std::vector<std::vector<uint16_t>> &bytes, robin_hood::unordered_map<std::pair<uint16_t, uint16_t>, long long, PairHash> &freqs) {
  ...
  for (uint16_t it = 0; it < num_merges; it++) {
    ...
    freqs.erase(mf_vals); // no longer possible to get this key
  }
}
